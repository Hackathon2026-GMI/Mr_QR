...
Run with: python test_rigorous.py
"""
...
# ── Test definition ───────────────────────────────────────────────────────────
# Each test has:
#   url           — what the phone sends
#   label         — human description
#   expect        — "SAFE" | "SUSPICIOUS" | "DANGEROUS"  (or None to just observe)
#   expect_autofill   — True/False/None
#   expect_field_map  — True (non-empty) | False (empty) | None (don't check)
#   expect_keys       — list of vault keys that MUST appear in required_vault_keys
...
TESTS = [
    # ── Group A: Real official FIFA/WC2026 domains ───────────────────────────
    {
        ...
        "expect_field_map":  False,   # homepage has no forms
        "expect_keys":       [],
    },
    {
    ...
    # ── Group B: Real clean non-FIFA domains ─────────────────────────────────
    {
        ...
        "expect":            "SUSPICIOUS",  # not an official FIFA entity
    ...
    # ── Group C: Mock server — controlled HTML scenarios ─────────────────────
    {
        ...
        "expect":            "SUSPICIOUS",  # localhost is not official, but forms map
        "expect_autofill":   False,
        "expect_field_map":  True,          # MUST return a non-empty field_map
        "expect_keys":       ["passport_id", "first_name", "last_name",
                              "nationality", "fifa_fan_id", "email"],
    },
    {
        "label":             "C2 — Mock: phishing form (passport + payment on fake domain)",
        "url":               "http://localhost:9999/phishing-form",
        "expect":            "DANGEROUS",
        "expect_autofill":   False,
        "expect_field_map":  False,         # field_map must be empty on DANGEROUS
        "expect_keys":       [],
    },
    {
    ...
    {
        "label":             "C4 — Mock: redirect chain → phishing form",
    ...
    {
        "label":             "D2 — Google Safe Browsing test malware page",
    ...
    {
        "label":             "D3 — HTTP (not HTTPS) on official-looking domain",
        "url":               "http://www.fifa.com",
        "expect":            "SAFE",   # fifa.com redirects to HTTPS — final URL is safe
        "expect_autofill":   True,
        "expect_field_map":  None,
        "expect_keys":       [],
    },
...
async def main():
    ...
    print(f"  Mock server expected at: http://localhost:9999")
    print(f"  Orchestrator expected at: http://localhost:8000\n")

    results = []
    for i, test in enumerate(TESTS, 1):
        r = await run_test(test, i, len(TESTS))
        results.append(r)
        # Brief pause between tests to stay within Gemini per-minute rate limits
        if i < len(TESTS):
